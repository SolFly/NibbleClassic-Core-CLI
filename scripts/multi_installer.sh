...
# nibble Multi-installer
# a one line clone-and-compile for nibble:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/nibble/nibble/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/nibble ]; then
            read -r -p "${1:-nibble directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old nibble directory" && echo
                    rm -rf "$PWD"/nibble
                    ;;
                *)
                    _fail "nibble directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir nibble
        git clone -b master -q https://github.com/nibble/nibble nibble   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd nibble
    fi
}

_build_nibble() {
    _note "Building nibble from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/nibble')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/nibble')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "nibble Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'nibble/build/src/' for the executible binaries. See 'https://github.com/nibble/nibble' for more project support. Cowabunga!"
